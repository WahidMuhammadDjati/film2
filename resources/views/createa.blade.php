        ...
        <table class="w-full border-collapse border border-gray-300">
            ...
            <tbody>
                @foreach ($aktors as $aktor)
                    <tr>
                        {{-- <td class="border border-gray-300 p-2">{{ $aktor->id }}</td> --}}
                        <td class="border border-gray-300 p-2">{{ $aktor->nama_aktor }}</td>
                        <td class="border border-gray-300 p-2 space-x-2">
                            ...
                            <form action="{{ route('aktor.destroy', $aktor->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
