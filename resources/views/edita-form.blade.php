@if(isset($aktor))
    <div x-data="{ open: false }">
        <button @click="open = true" class="bg-blue-500 text-white px-4 py-2 rounded">Edit Aktor</button>

        <div x-show="open" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-white p-4 rounded-lg shadow-lg w-full max-w-3xl">
                <h2 class="text-lg font-semibold mb-3 text-center">Edit Aktor</h2>
                <form action="{{ route('aktor.update', $aktor->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="block text-sm">Nama:</label>
                        <input type="text" name="nama_aktor" value="{{ $aktor->nama_aktor }}" class="w-full border p-2 rounded-md" required>
                    </div>
